...
class Pos():
    ...
    def __init__(self, a, b,X2,func, Nn, position="max", c1=2.0, c2=2.0, w=0.3, adm=False, vmin=1, vmax=5):
        self.X2=X2
        self.a = a
        self.b = b
        self.func = func
    ...
    def fit(self, num=1000):
        ...
        for i in range(0, num):
            ...
            x = x + v
            x = self.overstep(x)
            pbest=self.overstep(pbest)
            pf = self.func(pbest.T,self.X2)
            f = self.func(x.T,self.X2)
            for i in range(0, len(f)):
                if pf[i] < f[i]:
                    pbest[i] = x[i]
            gbest = pbest[np.argmax(pf)]
            self.g.append(gbest)
            self.ff.append(self.func(gbest,self.X2))
